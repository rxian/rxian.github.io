---
layout: default
title: Ruicheng Xian
---

<style>
#bio-header-container h1 {
  font-size: 1.6em;
  margin-bottom: 3px;
}
h2 {
  font-size: 1.2em;
}
#bio-header-picture-container img {
  float: right;
  /* max-width: 30%; */
  min-width: 180px;
  width: 180px;
  margin-left: 5%;
  margin-bottom: 5%;
  border-radius: 5px;
}
@media screen and (max-width: 550px) {
  #bio-header-picture-container img {
    float: none;
    max-width: 100%;
    min-width: inherit;
    width: 120px;
    margin-left: 0%;
    margin-bottom: 10px;
    padding-left: auto;
    padding-right: auto;
  }
}
.block { padding: 0 15px 0 15px; }
.publications-block p {
  margin-left: 15px;
  text-indent: -15px;
}
.publications-block p:last-child {
  margin-bottom: 0;
}
.publications-block {
  border: 1px;
  border-style: solid;
  border-color: #dfe0e5;
  margin-top: 20px; 
  padding: 15px 15px 15px 15px;
  border-radius: 5px;
}
header { }
article { 
  flex: 1;
  padding-top: 30px;
}
footer { 
  /* text-align: right; */
  /* margin-top: 10px; */
}
.wrapper { 
  height: 100vh;
  display: flex;
  flex-direction:column;
}
.page-content { padding: 0; }
</style>

<article>

<div class="block" style="padding-right: 0" markdown="1">

<div id="bio-header-container" markdown="1">

<div id="bio-header-picture-container" markdown="1">

![Photo of myself](assets/spice.jpg)

</div>

<h1 style="line-height: 0.8em;">Ruicheng Xian</h1>

<h1 style="margin-bottom: 5px; font-size: 1.5em; font-weight: 400;">冼睿成</h1>

rxian2(at)illinois.edu  &nbsp;·&nbsp; [google scholar](https://scholar.google.com/citations?user=Nmk26z4AAAAJ&sortby=pubdate)

</div>

</div>

<div class="block" markdown="1">

---

I am a fourth-year [CS](http://cs.illinois.edu) PhD student at the University of Illinois Urbana-Champaign, where I am fortunate to be advised by the great [Han Zhao](http://hanzhaoml.github.io), and to have worked with the glorious [Matus Telgarsky](http://mjt.web.engr.illinois.edu).  Previously, I graduated from the [ECE](http://ece.illinois.edu) department with a BS degree.

My interests lie broadly in machine learning, transfer learning, and natural language processing.  I currently study fairness and domain adaptation.

</div>

<div class="block publications-block" markdown="1">

## Papers

Learning List-Level Domain-Invariant Representations for Ranking  
Ruicheng Xian, Honglei Zhuang, Zhen Qin, Hamed Zamani, Jing Lu, Ji Ma, Kai Hui, Han Zhao, Xuanhui Wang, Michael Bendersky  
[Preprint, 2022](https://arxiv.org/abs/2212.10764) &nbsp;·&nbsp; [pdf](assets/xian2023listlevelinvariant.pdf)

Fair and Optimal Classification via Post-Processing Predictors  
Ruicheng Xian, Lang Yin, Han Zhao  
[Preprint, 2022](https://arxiv.org/abs/2211.01528) &nbsp;·&nbsp; [pdf](assets/xian2023fairoptimalclassification.pdf) &nbsp;·&nbsp; [code](https://github.com/rxian/fair-classification)

Cross-Lingual Transfer with Class-Weighted Language-Invariant Representations  
Ruicheng Xian, Heng Ji, Han Zhao  
[ICLR 2022](https://openreview.net/forum?id=k7-s5HSSPE5) &nbsp;·&nbsp; [pdf](assets/xian2022crosslingual.pdf) &nbsp;·&nbsp; [code](https://github.com/rxian/domain-alignment)

Neural tangent kernels, transportation mappings, and universal approximation  
Ziwei Ji, Matus Telgarsky, Ruicheng Xian  
[ICLR 2020](https://openreview.net/forum?id=HklQYxBKwS) &nbsp;·&nbsp; [pdf](https://arxiv.org/pdf/1910.06956.pdf)

</div>

<div class="block" style="text-align: right; margin-top: 10px; color: #afafb6; font-size: 0.85em;" markdown="1">

February 2023

</div>

</article>

<!-- 
<footer>

<div class="block" markdown="1">

October 2022

</div>

</footer>
-->
